...
from custom_layer import NonLinearLayer, function
...
class MultiLayerArtificial(Sequential):
    """Multi layer perceptron with additional variational quantum circuit layer."""
    def __init__(
        self,
        function=function,
        ...
        for i in range(n_layers):
            if i == 0:
                ...
                layers.append(Linear(n_units[i - 1], n_units[i]))
            layers.append(deepcopy(activation))
        layers.append(Linear(n_units[-1], function_inputs))
        layers.append(deepcopy(activation))
        layers.append(NonLinearLayer(function, function_params))
        layers.append(Linear(function_outputs, out_features))
        layers.append(deepcopy(out_activation))
